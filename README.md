...
## CLOB V2 — важно перед включением LIVE

Polymarket в 2026 мигрировал торговый API на CLOB V2 и архивировал старый
клиент `py-clob-client` — он теперь отдаёт `invalid order version, please
use the latest clob-client` на любой ордер. Бот использует актуальный
`py-clob-client-v2`.

**Но само API V2 на момент миграции ещё нестабильно** — в официальном
репозитории Polymarket открыты баги, из-за которых ордера отклоняются с
`maker address not allowed, please use the deposit wallet flow` для целого
ряда типов кошельков (обычный EOA/MetaMask, email/Magic-link прокси,
старые V1 Safe-кошельки) — это подтверждено самими разработчиками
Polymarket, чинить на нашей стороне тут нечего, вопрос когда исправят у
них. Если после апдейта LIVE-ордер всё равно падает с этой ошибкой —
это не баг бота, а известная проблема Polymarket на твоём типе кошелька.
...
- Держи `DRY_RUN=true`, пока не подтвердишь, что реальный ордер на твоём
  ...
  ставит FOK-ордер по заведомо невыгодной цене (0.01), который почти
  наверняка не исполнится, но покажет, доходит ли запрос до сервера без
  ошибки авторизации/адреса. Запуск: `python -m scripts.test_live_order`
  (нужен заполненный `.env`).
- Если используешь email/Magic-логин на Polymarket — риск упереться в этот
  баг выше всего. Самообслуживаемый кошелёк (MetaMask, `POLY_SIGNATURE_TYPE=0`)
  исторически стабильнее, но и он не застрахован на момент миграции.
- Проверяй issues в https://github.com/Polymarket/py-clob-client-v2/issues
  на предмет статуса — это активно чинящаяся ситуация.
